<template>
  ...
  <div class="datepicker-container">
    ...
    <OldMapModal :isOpen="isModalOpen" :shipData="items" @close="closeModal" />
  </div>
</template>
      ...
      if (data.startsWith('*')) {
        ...
        if (msgId && this.markers[mmsi] && trueHead) {
          ...
          if (isNaN(currentPosition[0]) || isNaN(currentPosition[1]) ||
              isNaN(futurePosition[0]) || isNaN(futurePosition[1])) {
            console.error("Invalid coordinates:", currentPosition, futurePosition);
            return;
          }
